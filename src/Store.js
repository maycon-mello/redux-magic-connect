...
import { take, fork, call, all, put, select } from 'redux-saga/effects';
import { combineReducers } from 'redux';
import { reduceState } from './magicConnect';
...
function* watchAction(action, fn, namespaces) {
  ...
  const namespacedSelect = (type, ...args) => select(state => reduceState(state, ns(type, namespaces), ...args));
  const rootSelect = (type, ...args) => select(state => reduceState(state, type, ...args));

  while (true) {
    const payload = yield take(ns(action, namespaces));
    yield call(
      fn,
      {
        put: namespacedPut,
